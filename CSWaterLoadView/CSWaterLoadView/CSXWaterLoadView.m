...
- (instancetype)initWithFrame:(CGRect)frame title:(NSString *)title font:(UIFont *)font showLoadNote:(BOOL)show{
    if (self = [super initWithFrame:frame]) {
        ...
        w = MAX(rect.size.height, rect.size.width) + 12;
        ...
        _crossLabel.backgroundColor = [UIColor lightGrayColor];
        ...
        _bottomLabel.backgroundColor = [UIColor colorWithRed:168/255.0 green:45/255.0 blue:27/255.0 alpha:1.0];
...
- (void)doAni
{
    ...
    //起始点
    CGFloat startY = _waveHeight*sinf(_offset*M_PI/_waveWidth) + (1.0-self._h)*w;
    CGPathMoveToPoint(pathRef, NULL, 0, startY);
    //第一个波的公式
    for (CGFloat i = 0.0; i < _waveWidth; i ++) {
        CGFloat y = 1.1*_waveHeight*sinf(_waveNum*M_PI*i/_waveWidth + _offset*M_PI/_waveWidth) + (1.0-self._h)*w;
    ...
    CGFloat startY2 = _waveHeight*sinf(_offset*M_PI/_waveWidth + M_PI/3)+(1.0-self._h)*w;
    CGPathMoveToPoint(pathRef2, NULL, 0, startY2);
    //第二个波曲线的公式
    for (CGFloat i = 0.0; i < _waveWidth; i ++) {
        CGFloat y = 1.1 *_waveHeight*sinf(_waveNum*M_PI*i/_waveWidth + _offset*M_PI/_waveWidth + M_PI/3) + (1.0-self._h)*w;
